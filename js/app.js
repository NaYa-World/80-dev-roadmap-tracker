...
function allIds() {
  const ids = [];
  PHASES.forEach((ph,pi) => ph.data.forEach((d,di) => d.tasks.forEach((_,ti) => ids.push(tid(pi,di,ti)))));
  return ids;
}
function cntDone()  { return allIds().filter(id => !!S[id]).length; }
...
function updateStats() {
  ...
  document.getElementById('s-streak').textContent = S._streak || 0;
  document.getElementById('prog').style.width     = pct + '%';
  document.getElementById('prog-lbl').textContent = pct + '%';
  document.getElementById('top-strip').style.transform = 'scaleX(' + (pct/100) + ')';

  // Tasks-left banner
  const msg = document.getElementById('left-msg');
  if (left === 0) {
    msg.innerHTML = '<strong>🎉 All tasks complete! You are ready to apply.</strong>';
  } else if (done === 0) {
    msg.innerHTML = '<strong>' + total + ' tasks</strong> ahead of you — start today, not tomorrow.';
  } else {
    const nextPhase = PHASES.find((ph,pi) =>
      ph.data.some((d,di) => d.tasks.some((_,ti) => !S[tid(pi,di,ti)]))
    );
    const npi = nextPhase ? PHASES.indexOf(nextPhase) : -1;
    const nextDay = nextPhase ? nextPhase.data.find((d,di) =>
      d.tasks.some((_,ti) => !S[tid(npi, di, ti)])
    ) : null;
    msg.innerHTML = '<strong>' + left + ' tasks left</strong> — next up: ' +
      (nextDay ? '<strong>' + nextDay.day + ' · ' + nextDay.label + '</strong>' : 'keep going!');
  }
}
...
function badge(k) {
  ...
  return '<span class="badge ' + (map[k]||'b-concept') + '">' + k + '</span>';
}

// ─── QUIZ CARD — built with DOM, no inline handlers with encoded strings ──────
function buildQuizCard(d, pi, di) {
  ...
  const claudeUrl = 'https://claude.ai/new?q=' + encodeURIComponent(msg);
  ...
  hdr.className = 'quiz-card-hdr';
  hdr.innerHTML =
    '<div class="quiz-card-icon">🤖</div>' +
    '<span class="quiz-card-name">Quiz me on: ' + d.label + '</span>' +
    '<span class="quiz-card-tag">AI quiz</span>';
  card.appendChild(hdr);

  // Prompt text (visible so user knows what they're copying)
  ...
  copyBtn.className = 'quiz-btn cp';
  copyBtn.innerHTML = '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg> Copy prompt';
  copyBtn.addEventListener('click', function() {
    ...
    btn.style.color = '#00d9a0'; btn.style.background = 'rgba(0,217,160,.08)';
    setTimeout(function() {
      btn.innerHTML = '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg> Copy prompt';
  ...
  linkBtn.className = 'quiz-btn op';
  linkBtn.innerHTML = '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg> Open in Claude ↗';
...
function render(filter, search) {
  ...
  PHASES.forEach(function(ph, pi) {
    ...
    const phTotal = ph.data.reduce(function(a,d){ return a + d.tasks.length; }, 0);
    const phDone  = ph.data.reduce(function(a,d,di){
      return a + d.tasks.filter(function(_,ti){ return !!S[tid(pi,di,ti)]; }).length;
    }, 0);
    ...
    card.style.setProperty('--pcd', ph.dim);

    // Phase header (HTML string, safe — no user content)
    const phHdrHTML =
      '<div class="phase-hdr" id="phdr'+pi+'">' +
        ...
        '<div style="flex:1;min-width:0;">' +
          ...
          '<div style="font-family:\'JetBrains Mono\',monospace;font-size:10px;color:var(--sub);">' + ph.days + ' · ' + phDone + '/' + phTotal + ' tasks</div>' +
          '<div class="mini-bar"><div class="mini-fill" style="width:'+phPct+'%;"></div></div>' +
        '</div>' +
        (isComp ? '<div class="done-pill">✓ complete</div>' :
          '<span style="font-family:\'JetBrains Mono\',monospace;font-size:10px;padding:3px 9px;border-radius:10px;background:var(--border);color:var(--sub);">' + phPct + '%</span>') +
    ...
    card.innerHTML = phHdrHTML +
      '<div class="phase-body'+(isOpen?' open':'')+'\" id="pb'+pi+'">' +
        '<div class="phase-body-inner" id="pbi'+pi+'"></div>' +
      '</div>';
    ...
    ph.data.forEach(function(d, di) {
      ...
      const show = !search || d.tasks.some(function(t){
        return t.t.toLowerCase().includes(sl) || d.label.toLowerCase().includes(sl) || d.day.toLowerCase().includes(sl);
      });
      if (!show) return;

      const dayDone = d.tasks.filter(function(_,ti){ return !!S[tid(pi,di,ti)]; }).length;
      const allDone = dayDone === d.tasks.length;
      const dayOpen = S['do'+pi+'_'+di] !== false; // day open state

      const dayBlock = document.createElement('div');
      dayBlock.className = 'day-block' + (allDone?' all-done':'');

      // Day dropdown header
      const dayHdr = document.createElement('div');
      dayHdr.className = 'day-hdr';
      dayHdr.innerHTML =
        '<span class="day-tag">' + d.day + '</span>' +
        '<span class="day-label">' + d.label + '</span>' +
        '<span class="day-count" style="color:' + (allDone?'#00d9a0':'var(--sub)') + ';">' +
          dayDone + '/' + d.tasks.length + (allDone?' ✓':'') +
        '</span>' +
      ...
      tasksWrap.className = 'day-tasks' + (dayOpen?' open':'');
      const tasksInner = document.createElement('div');
      tasksInner.className = 'day-tasks-inner';
      ...
      d.tasks.forEach(function(task, ti) {
        ...
        tx.className = 'tx' + (done?' done':'');
        tx.id = 'tx_'+id;
        tx.style.color = done ? 'var(--sub)' : 'var(--text)';
        tx.textContent = task.t;

        row.appendChild(cb);
        row.appendChild(tx);
        row.innerHTML += badge(task.k);

        (function(capturedPi, capturedDi, capturedTi) {
          row.addEventListener('click', function() { doTask(capturedPi, capturedDi, capturedTi); });
        })(pi, di, ti);

        tasksInner.appendChild(row);
      });

      // Quiz card (built with DOM, no inline handlers)
      tasksInner.appendChild(buildQuizCard(d, pi, di));
      tasksWrap.appendChild(tasksInner);

      // Day toggle click
      (function(capturedPi, capturedDi, capturedDayHdr, capturedTasksWrap) {
        capturedDayHdr.addEventListener('click', function() {
          const isNowOpen = capturedTasksWrap.classList.toggle('open');
          S['do'+capturedPi+'_'+capturedDi] = isNowOpen;
          save();
          const chev = capturedDayHdr.querySelector('.day-chev');
          if (chev) chev.classList.toggle('open', isNowOpen);
        });
      })(pi, di, dayHdr, tasksWrap);

      dayBlock.appendChild(dayHdr);
      dayBlock.appendChild(tasksWrap);
      inner.appendChild(dayBlock);
    });
...
// ─── ACTIONS ─────────────────────────────────────────────────────────────────
function doPhase(pi) {
  S['po'+pi] = S['po'+pi] === false ? true : false;
  save();
  const body = document.getElementById('pb'+pi);
  const chev = document.getElementById('ch'+pi);
  if (body) body.classList.toggle('open');
  if (chev) chev.style.transform = 'rotate(' + (body.classList.contains('open') ? 180 : 0) + 'deg)';
...
function doTask(pi, di, ti) {
  ...
  const ph  = PHASES[pi];
  if (cb) {
    cb.className = 'cb' + (S[id]?' on':'');
    cb.style.background  = S[id] ? ph.color : '';
    cb.style.borderColor = S[id] ? ph.color : '';
  }
  if (tx) { tx.className = 'tx'+(S[id]?' done':''); tx.style.color = S[id]?'var(--sub)':'var(--text)'; }
  if (row && S[id]) { row.classList.add('flash'); setTimeout(function(){ row.classList.remove('flash'); }, 400); }
  updateStats();
}

function resetAll() {
  S = {}; save(); updateStats(); render(curFilter, curSearch);
}
...
document.getElementById('search').addEventListener('input', function(e) {
  clearTimeout(st);
  st = setTimeout(function(){ curSearch = e.target.value; render(curFilter, curSearch); }, 200);
});
