...
function renderPhaseTabs() {
  ...
  if (!ptabs) return;
  let html = '<button class="pill' + (curFilter === 'all' ? ' active' : '') + '" data-phase="all">All phases</button>';
  PHASES.forEach(function (ph, pi) {
    const numIcon = ['①', '②', '③', '④', '⑤', '⑥', '⑦', '⑧'][pi] || (pi + 1);
    const titleShort = ph.title.includes('—') ? ph.title.split('—')[1].trim() : ph.title;
    html += '<button class="pill' + (curFilter === String(pi) ? ' active' : '') + '" data-phase="' + pi + '">' + numIcon + ' ' + titleShort + '</button>';
  });
...
function renderKanbanTabs() {
  ...
  if (!kf) return;
  let html = '<button class="pill' + (kbPhase === 'all' ? ' active' : '') + '" data-phase="all">All phases</button>';
  PHASES.forEach(function (ph, pi) {
    const icon = ph.icon;
    const titleShort = ph.title.includes('—') ? ph.title.split('—')[1].trim() : ph.title;
    html += '<button class="pill' + (kbPhase === String(pi) ? ' active' : '') + '" data-phase="' + pi + '">' + icon + ' ' + titleShort + '</button>';
  });
  kf.innerHTML = html;
}

function updateProgramHeaders() {
  const isDevops = getActiveProgram() === 'devops';
  const eyebrow = document.querySelector('#view-roadmap .eyebrow');
  const title = document.querySelector('#view-roadmap .page-title');
  const sub = document.querySelector('#view-roadmap .page-sub');
  if (eyebrow) eyebrow.textContent = isDevops ? 'Zero to Hired · 90 Days' : 'Zero to Hired · 80 Days';
  if (title) title.textContent = isDevops ? 'DevOps Roadmap Tracker' : 'Dev Roadmap Tracker';
  if (sub) sub.textContent = isDevops ? 'Linux · AWS · Docker · K8s · CI/CD · Terraform · 8–10 hrs/day' : 'React · TypeScript · Tailwind · DevOps · 8–10 hrs/day';
  renderPhaseTabs();
  renderKanbanTabs();
}
...
function initProgramSwitcher() {
  ...
  btn.innerHTML = current === 'devops' ? '🐧 DevOps ▾' : '🚀 Frontend ▾';
  menu.querySelectorAll('.prog-item').forEach(function (item) {
    const prog = item.dataset.prog;
    item.classList.toggle('active-prog', prog === current);
  });
  ...
  menu.querySelectorAll('.prog-item').forEach(function (item) {
    item.addEventListener('click', function () {
      ...
      btn.innerHTML = prog === 'devops' ? '🐧 DevOps ▾' : '🚀 Frontend ▾';
      menu.querySelectorAll('.prog-item').forEach(function (el) {
        el.classList.toggle('active-prog', el.dataset.prog === prog);
      });
...
function initMobileMenu() {
  ...
  document.addEventListener('click', function (e) {
    if (!navTabs.contains(e.target) && e.target !== menuBtn) {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (rb) rb.addEventListener('click', function () { if (confirm('Reset ALL learning progress? Jobs and Q-bank progress kept.')) { const jobs = S._jobs, qd = S._qdone, ap = S._activeProgram; S = {}; if (jobs) S._jobs = jobs; if (qd) S._qdone = qd; if (ap) S._activeProgram = ap; save(); updateStats(); showView(curView); } });
  // Pomodoro
  initPomo();
  // Job modal
  initJobModal();
