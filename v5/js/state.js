...
function load() {
  try {
    ...
    if (r) S = JSON.parse(r);
    if (S._activeProgram === 'devops') {
      PHASES = DEVOPS_PHASES;
    } else {
...
function getActiveProgram() { return S._activeProgram || 'frontend'; }
function setActiveProgram(prog) {
  S._activeProgram = prog;
  PHASES = prog === 'devops' ? DEVOPS_PHASES : FRONTEND_PHASES;
  save();
}

// ── IDs ──────────────────────────────────────
function tid(pi, di, ti) {
  const prefix = (S._activeProgram === 'devops') ? 'devops_' : '';
  return prefix + 'p' + pi + 'd' + di + 't' + ti;
}
...
// ── Notes ─────────────────────────────────────
function noteKey(pi, di) {
  const prefix = (S._activeProgram === 'devops') ? 'devops_' : '';
  return prefix + 'note_' + pi + '_' + di;
}
...
function confKey(pi, di, ti) {
  const prefix = (S._activeProgram === 'devops') ? 'devops_' : '';
  return prefix + 'conf_' + pi + '_' + di + '_' + ti;
}
