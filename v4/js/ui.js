...
function getCurrentVersion() {
  ...
  // Check for v2, v3, v4 in path
  const match = path.match(/\/(v\d+)/);
  return match ? match[1] : 'v4';
}
...
async function fetchVersions() {
  try {
    ...
    // Ensure v4 is always present even before GitHub indexes it
    if(!dirs.includes('v4')) dirs.push('v4');
    return dirs;
  } catch (err) {
    // Fallback: return all known versions statically
    return ['v2','v3','v4'];
  }
}
...
const VERSION_INFO = {
  ...
  'v2': { label:'v2 · +Kanban+Focus',    desc:'Kanban board + Daily Focus + Stats' },
  'v3': { label:'v3 · +AI+Pomodoro',     desc:'AI brief · Pomodoro · Notes · Jobs' },
  'v4': { label:'v4 · Latest',           desc:'Bigger UI · Polished version switcher' },
};
...
async function initVersionSwitcher() {
  ...
  const allVersions = ["v1", ...versions.filter(v => v !== "v1")];
  ...
  allVersions.forEach(v => {
    ...
    a.innerHTML =
      `<span style="flex:1;">`
        ...
        +`<div style="font-size:12px;color:var(--sub);margin-top:2px;">${info.desc}</div>`
      +`</span>`
      +(isCurrent
        ? `<span class="ver-badge current">current</span>`
        : v === "v4" ? `<span class="ver-badge new">latest</span>` : "");
